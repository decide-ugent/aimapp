...
from map_dm_nav.visualisation_tools import pickle_load_model, create_save_data_dir, pickle_dump_model, save_step_data,save_failed_step_data
...
class AIFProcessServer(Node):
    ...
    def __init__(self, test_id:int='None'):
        ...
        ideal_next_action, data = self.model.define_actions_from_MCTS_run(num_steps=1, observations=[self.last_ob_id],next_possible_actions=next_possible_actions, save_action_memory = False)
    ...
    def execute_cb(self, goal_handle):
        ...
        if goal_success is False:
            ...
            ideal_next_action, data = self.model.define_actions_from_MCTS_run(num_steps=1, observations=[self.last_ob_id],next_possible_actions=possible_actions, save_action_memory = False)
            save_failed_step_data(copy.deepcopy(self.model), self.last_ob_id, np.array([0,0]), [0], possible_actions, self.prev_scans_dist, self.robot_pose, action_success=False, elapsed_time=0, store_path=self.test_folder, action_select_data=data)
        ...
        self.model.set_action_step(goal_action)
        ...
        next_possible_actions = self.model.define_next_possible_actions(obstacle_dist_per_actions, restrictive=True,logs=self.get_logger())
        ideal_next_action, data = self.model.define_actions_from_MCTS_run(num_steps=1, observations=[ob_id],next_possible_actions=next_possible_actions, save_action_memory = False)
        self.get_logger().info(f'THE IDEAL NEXT MOTION (FOR MCTS):{ideal_next_action}')
    ...
    def save_data_process(self, ob_id:int, ob_match_score:list,\
        ...
        save_step_data(self.model, ob_id, ob, ob_match_score, obstacle_dist_per_actions,\
                    robot_pose, action_success=True, elapsed_time=elapsed_time,\
                        store_path=self.test_folder, action_select_data=data)
