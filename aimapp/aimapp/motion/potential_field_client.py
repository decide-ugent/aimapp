...
from aimapp_actions.action import PotentialField    
from action_msgs.msg import GoalStatus

class PFClient(Node):

    def __init__(self):
        super().__init__('potential_field_client')
        self.get_logger().info('potential_field_client node has been started.')
        # self.cli = self.create_client(PotentialField, 'potential_field')    
        self.cli = ActionClient(self,PotentialField, 'potential_field')     
...
def main(x,y):
    ...
    potential_field_client = PFClient()

    pose = Point(x=x, y=y)

    future = potential_field_client.send_goal(pose)
    
    rclpy.spin_until_future_complete(potential_field_client, future)
    potential_field_client.get_logger().info(
                'Goal reached with potential field')
    # %s' %  str(response.status))

    potential_field_client.destroy_node()
    rclpy.shutdown()
