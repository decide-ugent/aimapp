...
from map_dm_nav.visualisation_tools import pickle_load_model
...
class NodeVisualizer(Node):
    ...
    def __init__(self):
        ...
        # Get the use_sim_time parameter (automatically declared by ROS2)
        use_sim_time = self.get_parameter('use_sim_time').get_parameter_value().bool_value
        self.get_logger().info(f'Node Visualizer starting with use_sim_time: {use_sim_time}')
    ...
    def load_current_model(self) -> Optional[any]:
        ...
        try:
            ...
            # In Docker container: NodeVisualizer is in /home/ros2_ws/src/map-dm-nav/map-dm-nav/map-dm-nav/obs_transf/
            # high_nav_results is in: /home/ros2_ws/src/map-dm-nav/
            results_dir = Path("/home/ros2_ws/src/map-dm-nav/tests")
            ...
            if not results_dir.exists():
                self.get_logger().warning("tests directory not found")
                return None
    ...
    def create_connection_markers(self, agent_state_mapping: Dict, B_matrix: np.ndarray, 
                                 possible_actions: Dict, pose_dist: float, 
        ...
        Args:
            ...
            possible_actions: Dictionary of possible actions {action_id: [start_angle, end_angle] or "STAY"}
        ...
        # Create direction mapping from action_id to motion vector
        direction_mapping = {}
        for action_id, action_def in possible_actions.items():
            if action_def == 'STAY':
                direction_mapping[action_id] = (0, 0)
            else:
                # Take the middle angle of the range
                start_angle, end_angle = action_def
    ...
    def update_visualization(self):
        ...
        try:
            ...
            try:
                ...
                possible_actions = model.get_possible_actions()
                pose_dist = model.influence_radius
            ...
            try:
                node_markers = self.create_node_markers(agent_state_mapping, frame_id='odom')
                ...
                if B_matrix is not None and possible_actions is not None and pose_dist is not None:
                    connection_markers = self.create_connection_markers(
                        agent_state_mapping, B_matrix, possible_actions, pose_dist
                    )
                ...
                if current_pose is not None:
                    robot_marker = self.create_robot_pose_marker(current_pose, frame_id='odom')
